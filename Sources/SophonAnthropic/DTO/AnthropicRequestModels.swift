...
struct AnthropicRequest: Encodable {
    ...
    static func make(
        ...
        let turns = messages.filter { $0.role != .system }.map { message in
...
enum AnthropicContentBlock: Encodable {
    ...
    init(_ part: LLMPart) {
        ...
        case let .inlineData(mimeType, data):
            self = mimeType.hasPrefix("image/") ? .image(mediaType: mimeType, data: data) : .document(mediaType: mimeType, data: data)
        }
    }
